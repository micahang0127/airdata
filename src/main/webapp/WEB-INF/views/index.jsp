...
.pm-title {
    ...
    top: 0px;
    font-size: 1.15em;
    transform: translate(0%, -40%) scale(1, 1.2);
}
.pm-bar-elem {
    position: relative;
    ...
    $(function() {
        ...
        $.ajax({
            ...
            success : function(aa) { // aa(맘대로 지정)에는 해당 controller에서 return값이 들어있다.
                var loc = $('.all').empty();
                var templet = '<div class="thumbnail pm-box" style="background-color: {backColor}; border-color: {borderColor}"><h4><a href="/airdata/region/rt/[sido]" id="sido">{sido}</a></h4><p id="grade" style="color: {color}">{grade}</p><p id="pm10"><b>{pm10}</b>[10]</p><p id="pm25"><b>{pm25}</b>[2.5]</p></div>';
                var tem = aa;
                for (i = 0; i < sido.length; i++) {
                    ...
                    for (j = 0; j < srcData.length; j++) {
                        ...
                        if (sido[i] == srcData[j].region) {
                            ...
                            var html = templet.replace('{backColor}',gradePm10(srcData[j].avgPm10).backColor)
                            					.replace('{borderColor}', gradePm10(srcData[j].avgPm10).color)
                            					.replace('[sido]',sido[i])
                                                .replace('{sido}',sido[i])
...
<body>
    <form action="/index" method="GET">
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-xs-12 col-sm-4"><h2><span class="glyphicon glyphicon-hourglass" aria-hidden="true"></span>&nbsp;실시간 현황</h2></div>
                <div class="col-xs-12 col-sm-8">
                    <ul class="progress pm-bar">
                        <li class="pm-title">PM 2.5</li>
                        <li class="progress-bar progress-bar-success pm-bar-elem" style="width: 25%; background-color: blue;">
                            <span class="pm-bar-val">0~15</span>
                        </li>
                        <li class="progress-bar progress-bar-warning progress-bar-striped pm-bar-elem"
                            style="width: 25%; background-color: #2ace0a;">
                            <span class="pm-bar-val">16~35</span>
                        </li>
                        <li class="progress-bar progress-bar-danger pm-bar-elem" style="width:25%; background-color: yellow;">
                            <span class="pm-bar-val">36~75</span>
                        </li>
                        <li class="progress-bar progress-bar-danger pm-bar-elem" style="width:25%; background-color: #FF0000;">
                            <span class="pm-bar-val-max">76~</span>
                        </li>
                        <li id="pm25-mark" class="pm-mark"></li>
                    </ul>
                    <ul class="progress pm-bar">
                        <li class="pm-title">PM 10</li>
                        <li class="progress-bar progress-bar-success pm-bar-elem" style="width: 25%; background-color: blue;">
                            <span class="pm-bar-val">0~30</span>
                            <span class="pm-bar-txt">좋음</span>
                        </li>
                        <li class="progress-bar progress-bar-warning progress-bar-striped pm-bar-elem"
                            style="width: 25%; background-color: #2ace0a;">
                            <span class="pm-bar-val">31~80</span>
                            <span class="pm-bar-txt">보통</span>
                        </li>
                        <li class="progress-bar progress-bar-danger pm-bar-elem" style="width:25%; background-color: yellow;">
                            <span class="pm-bar-val">81~150</span>
                            <span class="pm-bar-txt">나쁨</span>
                        </li>
                        <li class="progress-bar progress-bar-danger pm-bar-elem" style="width:25%; background-color: #FF0000;">
                            <span class="pm-bar-val-max">151~</span>
                            <span class="pm-bar-txt">매우나쁨</span>
                        </li>
                        <li id="pm10-mark" class="pm-mark"></li>
                    </ul>
                </div>
            </div>
            <div class="row">
                <div class="col-xs-12"><p>&nbsp;&nbsp;&nbsp;※각 지역의 평균값으로 표시됩니다.(등급 pm10기준)</p></div>
                <div class="col-xs-12">
                    <div class="all">
                        <!--
